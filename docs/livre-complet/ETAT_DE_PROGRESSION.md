...
4. **Volume 11-12** : Reste du back-end Niveau 2 et API/communications réseau — découpé en sous-chapitres :
   ...
   - 11z-2 : Production (planning, productions enregistrées, bons de livraison) *(prochain)*
   - 11z-3 : Départements/Groupes, Zones dépositaires, Clients
   - 11z-4 : Notifications, État système, Paramètres (boutique), Premier lancement
   - 11z-5 : À propos, Assistant, Export, Rapports, Rapports personnels
   - Volume 12 : Socket.io et communications réseau (`lib/realtime.ts`, `lib/events.ts`, `services/notifications.ts`, `lib/socket.tsx`, `NotificationBell.tsx`, `ActivityFeed.tsx`)
...
`apps/web/src/pages/Produits.tsx` (Volume 11z-1), dernier des 7 fichiers du sous-chapitre Stocks/Fournisseurs/Produits — tous marqués « Vérifié ».

## 5. Prochaine tâche exacte

Rédiger le **Volume 11z-2 — Production** : `apps/api/src/routes/production.ts` (planning du jour suivant, productions enregistrées avec décrémentation de stock via `appliquerMouvement` en sens `SORTIE` — référence croisée directe avec `services/stocks.ts`, déjà couvert au 11z-1 —, service PDF du bon de livraison déjà mentionné au Volume 18/futur), `apps/web/src/pages/Production.tsx`, `apps/web/src/pages/BonsLivraison.tsx`. Lire ces 3 fichiers en entier avant de rédiger — aucun n'a encore été ouvert dans le cadre du livre. Puis enchaîner sur 11z-3 (Départements/Groupes, Zones dépositaires, Clients), 11z-4 (Notifications, État système, Paramètres, Premier lancement) et 11z-5 (À propos, Assistant, Export, Rapports) avant le Volume 12 (Socket.io/réseau) — voir §3 pour le découpage complet.
...
| Niveau 2 | 13 / 66 (`apps/api/src/lib/contexteRequete.ts`, `apps/api/src/routes/audit.ts`, `apps/web/src/pages/Audit.tsx`, `apps/api/src/app.ts`, `apps/web/src/App.tsx`, `apps/web/src/components/Layout.tsx`, `apps/api/src/routes/stocks.ts`, `apps/api/src/services/stocks.ts`, `apps/api/src/routes/fournisseurs.ts`, `apps/api/src/routes/produits.ts`, `apps/web/src/pages/Stocks.tsx`, `apps/web/src/pages/Fournisseurs.tsx`, `apps/web/src/pages/Produits.tsx`) | ~20 % |
