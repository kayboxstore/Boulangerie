...
3. **Volumes 7-10** : Arborescence détaillée, cycle de démarrage, UI et composants (Niveau 2/3), navigation et gestion d'état. *(prochain)*
...
`README.md` (Volume 4), `.env.example` et `render.yaml` (Volume 5) — tous marqués « Vérifié ». Prérequis d'installation et tests unitaires vérifiés directement dans l'environnement de rédaction (`npx prisma validate`, `npm test` — 11 tests passés) ; les étapes nécessitant une base de données active (migration, seed, démarrage complet) n'ont pas pu être vérifiées de bout en bout ici, faute de PostgreSQL accessible — signalé explicitement dans le chapitre plutôt que supposé.

## 5. Prochaine tâche exacte

Rédiger les **Volumes 7-10** : Volume 7 (Arborescence détaillée du projet, avec renvoi vers `INVENTAIRE_DU_PROJET.md` pour le détail fichier par fichier), Volume 8 (Cycle de démarrage : `apps/api/src/index.ts` → `app.ts` → routers → écoute HTTP côté serveur ; `main.tsx` → `App.tsx` côté frontend, avec diagramme de séquence Mermaid), Volume 9 (Interface utilisateur et composants Niveau 2/3 : système de design Tailwind/Radix, `CarteLigne` vs `Table`, inventaire commenté des composants réutilisables), Volume 10 (Navigation et gestion de l'état : arbre de routes, gardes `RequiertLecture`/`RequiertEcriture`, clés de requête TanStack Query). Avant de rédiger : lire `apps/api/src/index.ts`, `apps/api/src/app.ts`, `apps/web/src/main.tsx`, `apps/web/src/App.tsx` en entier — aucun de ces fichiers n'a encore été ouvert dans le cadre du livre.
...
| Niveau 3 | 5 / 63 (`apps/api/src/lib/prisma.ts`, `prisma/seed.ts`, `prisma/migrations/*.sql`, `render.yaml`, `.env.example`) | ~8 % |
| **Global** | **~33 / 155** | **~21 %** |
