...
1. **Volumes Niveau 1 restants** (priorité absolue, dans cet ordre suggéré car chaque fichier s'appuie sur les précédents) :
   ...
   - `apps/api/src/routes/travailleurs.ts` (985 lignes — probablement scindé en plusieurs chapitres : fiche/pointage, absences/sanctions, salaire/bulletins) + `apps/web/src/pages/Travailleurs.tsx` + `apps/web/src/components/PaieCard.tsx` *(prochain — dernier chapitre Niveau 1)*
...
`apps/api/src/routes/caisse.ts` et `apps/web/src/pages/Caisse.tsx` (chapitre 11j) — tous deux marqués « Vérifié » (le fichier partagé `packages/shared/src/index.ts` reste « En cours » dans son ensemble, voir §6). Confirmé : le registre n'a aucune table dédiée, `ClotureCaisse`/`Vente`/`LigneVente` restent orphelines, aucune référence à ces tables dans `caisse.ts`.

## 5. Prochaine tâche exacte

Rédiger le chapitre **11k — Travailleurs et Paie**, couvrant `apps/api/src/routes/travailleurs.ts` (985 lignes — le plus gros fichier de route du projet, probablement à scinder en plusieurs sous-chapitres : ex. `11k-1` fiche/pointage, `11k-2` absences/sanctions, `11k-3` salaire/bulletins de paie), `apps/web/src/pages/Travailleurs.tsx` et `apps/web/src/components/PaieCard.tsx`. C'est le **dernier chapitre Niveau 1** prévu par l'ordre de priorité — une fois ce chapitre posé (ou ses sous-chapitres), les 26 fichiers Niveau 1 seront tous couverts et le travail pourra basculer entièrement sur `prisma/schema.prisma` (Volume 13, ERD complet) puis sur les 66 fichiers Niveau 2 restants. Avant de rédiger : lire `apps/api/src/routes/travailleurs.ts` en entier (probablement en plusieurs passes vu sa taille), identifier ses sections naturelles (la spec section 3.18, déjà partiellement citée au Volume 11f pour le rappel d'absence en attente, devra être lue dans son intégralité — elle couvre fiche, pointage, absences avec justification, sanctions, et paie/bulletins avec règles de calcul, arrondi et gel des bulletins déjà émis, cette dernière partie étant explicitement listée dans le mandat comme nécessitant un traitement Niveau 1 complet). Découper le découpage exact des sous-chapitres seulement après cette première lecture complète, pas avant.
...
*(`packages/shared/src/index.ts` est compté comme « partiellement vérifié » : ses fonctions financières, de permission, de délégation, d'actions critiques, d'audit, de commandes, de commissions et de caisse sont couvertes en détail, mais le fichier dans son ensemble reste « En cours » tant que ses autres sections n'ont pas été traitées au fil des chapitres correspondants — voir §6. Les 20 autres fichiers Niveau 1 des chapitres 11b à 11j sont, eux, intégralement couverts et comptés comme pleinement « Vérifié ».)*

Ce pourcentage progresse régulièrement : dix chapitres Niveau 1 complets (11a à 11j) sont maintenant posés. Plus de quatre fichiers Niveau 1 sur cinq sont désormais couverts — il ne reste que le grand chapitre Travailleurs/Paie (11k, probablement scindé en plusieurs sous-chapitres vu ses 985 lignes) avant de clore intégralement le Niveau 1 et de passer au Volume 13 (base de données) puis au Niveau 2. Le compte réel « 21 / 26 » ci-dessus arrondit `packages/shared/src/index.ts` à une unité complète malgré sa couverture partielle, par simplicité d'affichage — la nuance reste documentée dans cette note.
