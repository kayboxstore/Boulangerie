...
1. **Volumes Niveau 1 restants** (priorité absolue, dans cet ordre suggéré car chaque fichier s'appuie sur les précédents) :
   ...
   - `apps/api/src/routes/commissions.ts` + `apps/web/src/pages/Commissions.tsx` *(prochain)*
...
`apps/api/src/routes/commandes.ts` et `apps/web/src/pages/Commandes.tsx` (chapitre 11h) — tous deux marqués « Vérifié » (le fichier partagé `packages/shared/src/index.ts` reste « En cours » dans son ensemble, voir §6). Confirmé au passage : les écrans satellites (fiche Client, Schéma de commande, Bon de livraison, Zones de dépôt) restent hors de ce chapitre, renvoyés au Volume 18 (Niveau 2).

## 5. Prochaine tâche exacte

Rédiger le chapitre **11i — Commissions**, couvrant `apps/api/src/routes/commissions.ts` et `apps/web/src/pages/Commissions.tsx`. Ce chapitre n'a pas encore été lu — il faudra d'abord l'ouvrir en entier. D'après le Glossaire et le Volume 11a, une commission est calculée automatiquement pour chaque commande d'un client de Qualité « Maman » (commissionParBac × quantiteBacs — formule à confirmer précisément dans le code, pas supposée). Le commentaire déjà repéré dans `packages/shared/src/index.ts` (Volume 11h, INDEX_DU_CODE) qualifie ce module de « vue dérivée des commandes Maman » — vérifier si `commissions.ts` recalcule quelque chose à la volée à partir de `CommandeClient` ou si un champ dédié stocke la commission par commande (aucun champ de ce type n'a été vu sur `CommandeClient` au Volume 11h — donc probablement un calcul à la lecture, à confirmer). Croiser avec la spec section 3.11.
...
*(`packages/shared/src/index.ts` est compté comme « partiellement vérifié » : ses fonctions financières, de permission, de délégation, d'actions critiques, d'audit et de commandes sont couvertes en détail, mais le fichier dans son ensemble reste « En cours » tant que ses autres sections n'ont pas été traitées au fil des chapitres correspondants — voir §6. Les 16 autres fichiers Niveau 1 des chapitres 11b à 11h sont, eux, intégralement couverts et comptés comme pleinement « Vérifié ».)*

Ce pourcentage progresse régulièrement : huit chapitres Niveau 1 complets (11a à 11h) sont maintenant posés, formant le socle sur lequel les chapitres suivants (commissions, caisse...) vont directement s'appuyer sans avoir à répéter les mécanismes déjà expliqués. Les deux tiers du Niveau 1 sont désormais couverts. Le compte réel « 17 / 26 » ci-dessus arrondit `packages/shared/src/index.ts` à une unité complète malgré sa couverture partielle, par simplicité d'affichage — la nuance reste documentée dans cette note.
