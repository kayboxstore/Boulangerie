...
1. **Volumes Niveau 1 restants** (priorité absolue, dans cet ordre suggéré car chaque fichier s'appuie sur les précédents) :
   ...
   - `apps/api/src/routes/commandes.ts` + `apps/web/src/pages/Commandes.tsx` *(prochain)*
...
`apps/api/src/lib/audit.ts`, `apps/api/src/lib/contexteRequete.ts`, `apps/api/src/lib/prisma.ts`, `apps/api/src/routes/audit.ts` et `apps/web/src/pages/Audit.tsx` (chapitre 11g) — tous marqués « Vérifié » (le fichier partagé `packages/shared/src/index.ts` reste « En cours » dans son ensemble, voir §6).

## 5. Prochaine tâche exacte

Rédiger le chapitre **11h — Commandes**, couvrant `apps/api/src/routes/commandes.ts` et `apps/web/src/pages/Commandes.tsx`. Ce chapitre peut réutiliser directement `calculerCommande` et `avanceAvantCommande`, déjà expliqués en détail au Volume 11a (avec exemple chiffré) — pas besoin de redériver la formule, seulement de montrer comment la route l'appelle, valide son entrée (Zod), gère le cas d'une commande dupliquée pour la même date/client, et comment le module Journal d'audit (11g, `MODELE_MODULE.CommandeClient = "COMMANDES"`) et le module Approbations (11f — vérifier si une opération de ce module fait partie des tâches critiques, a priori non listée dans les 5) s'y articulent. Vérifier aussi le lien avec le Schéma de commande et le Bon de livraison (spec section 3.3 d/e, déjà implémentés selon `INVENTAIRE_DU_PROJET.md`) — ce chapitre devra probablement se limiter au cœur `commandes.ts`/`Commandes.tsx` et renvoyer les écrans satellites (Schéma de commande, Bon de livraison, Clients, Zones de dépôt) à un ou plusieurs chapitres Niveau 2 ultérieurs (Volume 11z ou une sous-lettre dédiée), le Niveau de risque de ces satellites étant à confirmer dans `INVENTAIRE_DU_PROJET.md` avant de trancher leur traitement.
...
*(`packages/shared/src/index.ts` est compté comme « partiellement vérifié » : ses fonctions financières, de permission, de délégation, d'actions critiques et d'audit sont couvertes en détail, mais le fichier dans son ensemble reste « En cours » tant que ses autres sections n'ont pas été traitées au fil des chapitres correspondants — voir §6. Les 14 autres fichiers Niveau 1 des chapitres 11b à 11g sont, eux, intégralement couverts et comptés comme pleinement « Vérifié ».)*

Ce pourcentage progresse régulièrement : sept chapitres Niveau 1 complets (11a à 11g) sont maintenant posés, formant le socle sur lequel les chapitres suivants (commandes, commissions...) vont directement s'appuyer sans avoir à répéter les mécanismes déjà expliqués. Le chapitre 11g est le premier à avoir aussi fait progresser les Niveaux 2 et 3, en traitant au passage trois fichiers annexes (`contexteRequete.ts`, `routes/audit.ts`, `pages/Audit.tsx`, Niveau 2) et un fichier Niveau 3 (`lib/prisma.ts`) directement liés au mécanisme central du chapitre plutôt que de les reporter à un volume générique ultérieur. Le compte réel « 15 / 26 » ci-dessus arrondit `packages/shared/src/index.ts` à une unité complète malgré sa couverture partielle, par simplicité d'affichage — la nuance reste documentée dans cette note.
