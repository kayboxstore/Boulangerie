...
`reinitialiserBase(raison)` suit trois étapes strictement séquentielles : (1) `construireDump()` — si elle échoue, tout s'arrête immédiatement, **rien n'est effacé** ; (2) `ecrireSauvegardeLocale` — même règle, un échec d'écriture annule la réinitialisation ; (3) seulement alors, une longue liste de `deleteMany`/`updateMany` dans une **unique transaction Prisma**, dans l'ordre dicté par les contraintes de clé étrangère (enfants avant parents — Assistant, puis comptes-dépendant, Travailleurs, **Schémas de commande et Bons de livraison** (3.3 d/e — voir la correction ci-dessous), Commandes clients, Fournisseurs, Stocks, Production, Caisse (dont **remises et sessions**, mêmes précautions d'ordre), et enfin les comptes `Utilisateur` eux-mêmes, référencés par presque tout ce qui précède). Le catalogue `MatierePremiere` n'est pas supprimé, seul son `quantiteStock` est remis à 0 (`updateMany`) — conserver le catalogue évite de casser la décrémentation automatique de Production au prochain démarrage. Les Zones de dépôt (`ZoneDepositaire`), elles, sont délibérément **conservées** — traitées comme un référentiel organisationnel au même titre que les motifs de don, pas comme une donnée transactionnelle. Note technique : `deleteMany`/`updateMany` ne passent **pas** par l'extension d'audit (Volume 11g), qui n'intercepte que les opérations unitaires `update`/`delete` — sans conséquence ici puisque `AuditLog` lui-même fait partie des tables vidées dans la même transaction.

**Correction majeure de ce chapitre (plan d'action de l'audit du 19/08/2026)** : la description ci-dessus reflète la version corrigée du fichier. La version d'origine, décrite ici jusqu'à cette révision, **omettait** `SchemaCommande`/`SchemaCommandeLigne`, `BonLivraison`/`BonLivraisonLigne`, `CycleLivraison` (et sa cascade — Volume 11z-6), `SessionCaisse` et `RemiseCaisse` de cette séquence de suppression. Or `SchemaCommande.clientId`, `BonLivraison.clientId` et `CycleLivraison.commandeId` sont tous en `ON DELETE RESTRICT` (défaut Prisma pour les deux premiers, explicite pour le troisième) : dès qu'un Schéma de commande, un Bon de livraison ou un Cycle de livraison existait pour un client — la situation normale dès le premier jour d'usage réel de Production, pas un cas limite exotique — `prisma.commandeClient.deleteMany()` puis `prisma.client.deleteMany()` échouaient avec une violation de contrainte de clé étrangère. La transaction Prisma étant atomique, aucune donnée n'était jamais perdue lors de cet échec, mais **le bouton « Réinitialiser la base » ne fonctionnait tout simplement pas** en usage réel, alors que ce chapitre le présentait — de bonne foi, sur la base du code alors en vigueur — comme pleinement opérationnel. Vérifié de bout en bout après correction (créé un client avec Schéma + Cycle + Bon de livraison + Commande, plus une session de caisse et une remise, puis réinitialisé sans erreur, toutes les données transactionnelles vidées). C'est l'écart le plus significatif trouvé lors de la révision de ce livre du 19/08/2026 — un module dont **le comportement même**, pas seulement la documentation, était resté en retard sur l'évolution du reste du projet.
